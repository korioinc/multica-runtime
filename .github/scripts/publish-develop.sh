...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/runtime-develop.XXXXXX")
...
for platform in linux/amd64 linux/arm64; do
  ...
  build_id=$(jq -er '.config.Labels["io.multica.image-build-id"]' <<<"$metadata")
  [[ $seen_ids != *" $build_id "* ]] || fail 'native platforms must use distinct imageBuildIDs'
