...
[[ -n $version ]] || fail '--version must name the pushed release tag'
...
github_version_revision() {
  local requested_version=${1:-$version} require_tag=${2:-false} existing_release owner release_revision
  owner=$(github_tag_revision "$requested_version" "$require_tag") || return 1
  ...
  if [[ $existing_release != null ]]; then
    jq -e '.draft == false and .prerelease == false' \
      <<<"$existing_release" >/dev/null || fail 'GitHub release is incomplete'
    # GitHub ignores target_commitish when the tag already exists. The peeled
    # tag owns source identity; retain historical SHA evidence only if it is gone.
    if [[ -z $owner ]]; then
      release_revision=$(jq -er '.target_commitish | select(type == "string" and test("^[0-9a-f]{40}$"))' <<<"$existing_release") ||
        fail 'GitHub release without a tag has unknown source ownership'
...
plan_release() {
  local latest latest_identity latest_version latest_revision latest_owner latest_manifest manifest identity published_version=false
  guard || return 1
  ...
  if [[ $latest != null ]]; then
    ...
    # Check existing ownership without changing the version named by the pushed tag.
  ...
  [[ $manifest == null ]] || published_version=true
  guard || return 1
  jq -cnS --arg version "$version" --arg revision "$revision" --argjson published "$published_version" \
    '{version:$version,revision:$revision,published:$published}'
}
...
publish_release() {
  local manifest pin existing_release latest latest_identity latest_version compared latest_pin promote=true
  ...
  if [[ $existing_release == null ]]; then
    ...
    guard || return 1
    run_command 'promote GitHub release' gh release edit "$version" --repo "$GH_REPO" --latest >/dev/null || return 1
  fi
