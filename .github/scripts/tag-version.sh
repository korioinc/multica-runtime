...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/runtime-tag-version.XXXXXX")
trap 'rm -rf -- "$scratch"' EXIT
release_command='tag-version'
...
if [[ -z $owner ]]; then
  ...
  run_command 'create release tag' gh api --method POST "repos/$GH_REPO/git/refs" --input "$scratch/tag.json" >/dev/null
fi
...
run_command 'request tagged runtime release' gh api --method POST \
  "repos/$GH_REPO/actions/workflows/release.yml/dispatches" --input "$scratch/dispatch.json" >/dev/null
printf 'Requested runtime release %s at %s.\n' "$version" "$revision"
