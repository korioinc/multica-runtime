...
Update [VERSION](VERSION) to the next stable semantic release and merge the change
into `main`. The tagging workflow creates a tag at that commit and requests the
release workflow for the exact source revision. Tags follow `MAJOR.MINOR.PATCH`
without a prefix.
