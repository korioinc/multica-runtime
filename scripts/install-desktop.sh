...
# Reuse the pinned Debian repositories from install-os.sh, without rebuilding
# the language and package layers when desktop tools change.
...
# Use the versioned package and upstream digest instead of the mutable current URL.
chrome_package=$(basename -- "$(download_url google-chrome "$arch")")
download google-chrome "$scratch/$chrome_package"
(cd "$scratch" && sha256sum --check --ignore-missing /build-input/build/google-chrome.sha256)
...
(cd "$scratch" && sha256sum --check --ignore-missing /build-input/build/cua-driver.sha256)
