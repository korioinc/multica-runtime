...
runtime_input_keys() {
  case "$1" in
    ...
    packages)
      printf '%s\n' COREPACK_VERSION UV_VERSION YQ_VERSION SHFMT_VERSION GH_VERSION LEFTHOOK_VERSION \
    ...
    all)
      local scope
      for scope in controller os languages packages; do runtime_input_keys "$scope"; done ;;
