...
runtime_input_keys() {
  case "$1" in
    ...
    tools)
      printf '%s\n' UV_VERSION FD_VERSION YQ_VERSION SHFMT_VERSION GH_VERSION LEFTHOOK_VERSION \
        K9S_VERSION KUBECTX_VERSION KUBECTL_VERSION AWS_CLI_VERSION OCI_CLI_VERSION GCLOUD_CLI_VERSION ;;
    agents)
      printf '%s\n' COREPACK_VERSION MULTICA_CLI_VERSION CODEX_VERSION PI_VERSION PI_MCP_ADAPTER_VERSION PI_WEB_ACCESS_VERSION \
    ...
    database) printf '%s\n' MYSQL_CLI_VERSION POSTGRESQL_CLI_VERSION SQLITE_CLI_VERSION REDIS_CLI_VERSION ;;
    all)
      local scope
      for scope in controller os languages tools desktop database agents; do runtime_input_keys "$scope"; done ;;
...
runtime_versions_env() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$key" in
      MYSQL_CLI_VERSION|POSTGRESQL_CLI_VERSION|SQLITE_CLI_VERSION|REDIS_CLI_VERSION)
        [[ "$value" =~ ^([0-9]+:)?[0-9][0-9A-Za-z.+~-]*-[0-9A-Za-z.+~]+$ ]] || { echo "Invalid Debian client package version: $key" >&2; return 1; } ;;
...
runtime_npm_dependencies() {
  local group=$1
  local inputs
  inputs=$(runtime_versions_json agents) || return 1
